...
class WSProtocol(websockets.WebSocketCommonProtocol):
    ...
    def handshake(self, response):
        ...
        if subprotocols and ws_protocol:
            ...
            client_subprotocols = tuple((p.strip() for p in ws_protocol.split(",")))
    ...
    async def run(self):
        ...
        try:
            await self.request.app.hook("websocket_connect", self.request, self)
            ...
            await self.request.app.hook("websocket_disconnect", self.request, self)
